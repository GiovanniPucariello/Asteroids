...
import org.newdawn.asteroids.preferences.AsteroidsConstants;
...
public class GameWindow {
    ...
    public GameWindow() {
        try {
            ...
            DisplayMode mode = findDisplayMode(AsteroidsConstants.WINDOW_WIDTH.intValue(), AsteroidsConstants.WINDOW_HEIGHT.intValue(), currentBpp);

            // if can't find a mode, notify the user the give up
            if (mode == null) {
                Sys.alert("Error", "600x600x" + currentBpp + " display mode unavailable");
                return;
            }

            // configure and create the LWJGL display
            Display.setTitle("Asteroid Rush");
    ...
    /**
     * Add a game state to this window. This state can be used via its unique name.
     * 
     * @param state
    ...
    public void init() {
        ...
        GL11.glLoadIdentity();
        GLU.gluPerspective(45.0f, AsteroidsConstants.WINDOW_WIDTH / AsteroidsConstants.WINDOW_HEIGHT, 0.1f, 100.0f);
    ...
    /**
     * The main game loop which is cycled rendering and updating the registered game
     * states
     */
    ...
    /**
     * Change the current state being rendered and updated. Note if no state with the
     * specified name can be found no action is taken.
     * 
     * @param name
    ...
    /**
     * Enter the orthographic mode by first recording the current state, next changing us
     * into orthographic projection.
     */
    ...
    /**
     * Leave the orthographic mode by restoring the state we store in enterOrtho()
     * 
     * @see enterOrtho()
     */
    ...
    /**
     * The entry point into our game. This method is called when you execute the program.
     * Its simply responsible for creating the game window
     * 
     * @param argv
