...
public class SoundLoader {
    ...
    /**
     * Initialise the sound effects stored. This must be called before anything else will
     * work
     */
    public void init() {
        inited = true;
     ...
     *            The gain to play at
     */
    void playAsSound(int buffer, float pitch, float gain, boolean loop) {
        int play;
        if (soundWorks) {
            if (sounds) {
                nextSource++;
                ...
                play = AL10.alGetSourcei(sources.get(nextSource), AL10.AL_SOURCE_STATE);
