...
/**
 * The entity representing the player. This entity is responsible for displaying a model,
 * a particle system for the player's engine and for creating shot entities based on
 * player input.
 * 
...
public class Player extends AbstractEntity {
    ...
    /** The engine flare */
    private final ParticleGroup engine;
     ...
     * @param shotTexture
     *            The texture to apply to the shot's created when the player fires
     */
    ...
    public void update(EntityManager manager, int delta) {
        ...
        if (Keyboard.isKeyDown(Keyboard.KEY_LCONTROL) && Keyboard.isKeyDown(Keyboard.KEY_LSHIFT) && Keyboard.isKeyDown(Keyboard.KEY_T)) {
            toggleTestMode();
        }
        ...
        if (Keyboard.isKeyDown(Keyboard.KEY_UP) || Keyboard.isKeyDown(Keyboard.KEY_W) || Mouse.isButtonDown(1)) {
            ...
            engine.addParticle(positionX - (forwardX * flameOffset), positionY - (forwardY * flameOffset), 0.6f, 150);
        }
     ...
     * 
     * @param manager
     *            The EntityManager corresponding to the player (i.e. InGameState)
     */
    ...
    private void shotgun(EntityManager manager) {
        ...
        Shot leftshot = new Shot(shotTexture, getX() + forwardX, getY() + forwardY, forwardXL * 75, forwardYL * 75, 200, 0.9f, 0, 1, 0, true, 200, false);

        Shot rightshot = new Shot(shotTexture, getX() + forwardX, getY() + forwardY, forwardXR * 75, forwardYR * 75, 200, 0.9f, 0, 1, 0, true, 200, false);

        Shot centershot = new Shot(shotTexture, getX() + forwardX, getY() + forwardY, forwardX * 75, forwardY * 75, 200, 0.9f, 0, 1, 0, true, 200, false);
    ...
    private void shield(EntityManager manager) {
        ...
        Shield shield = new Shield(shotTexture, this, getX(), getY(), 25000, 3.5f, 0, 1, 1);
    ...
    private void laser(EntityManager manager) {
        ...
        Shot laser = new Shot(shotTexture, getX() + forwardX, getY() + forwardY, forwardX * 300, forwardY * 300, 150, 3f, 1, 0, 0, false, 350, false);
     ...
     * 
     * @param mode
     *            The current weapon equipped (0= default, 1=shotgun, 2=shield, 3=laser)
     * @param manager
     *            The EntityManager corresponding to the player (i.e. InGameState)
     */
    ...
    /**
     * Render the particle effect thats used to represent our ship's engine.
     */
     ...
     * 
     * @param mode
     *            The weapon to switch to (0=default, 1=shotgun, 2=shield, 3=laser)
     * @param manager
     *            The EntityManager corresponding to the player (i.e. InGameState)
     */
