...
/**
 * This state is responsible for rendering the game world and handling the
...
public class InGameState implements GameState, EntityManager {
	...
	private Texture shipTexture, shipTexture_2, shipTexture_3, shipTexture_4,
			shipTexture_5, shipTexture_6;
	...
	/**
	 * The current String associated with the amount of ammo for the equipped
	 * type
	 */
	...
	/**
	 * The current String associated with the duration of the shield_pickup
	 * remaining
	 */
	...
	public void init(GameWindow window) throws IOException {
		...
		music = SoundLoader.get().getOgg("res/synth_space.ogg");
	}

	/**
	 * @see org.newdawn.asteroids.GameState#render(org.newdawn.asteroids.GameWindow,
	 *      int)
	 */
	@Override
	...
	private void drawGUI(GameWindow window) {
		...
		if (life >= 0)
			font.drawString(1, lifeString, 795 - (lifeString.length() * 27), 5);
		if (ammoAmount.equals("0"))
			font.drawString(1, "NO AMMO", 5, 565);
		...
		font.drawString(1, level_string, 795 - (level_string.length() * 27),
				565);
	...
	/**
	 * @see org.newdawn.asteroids.GameState#update(org.newdawn.asteroids.GameWindow,
	 *      int)
	 */
	@Override
	 ...
	 * 
	 * @param count
	 *            The number of rocks to be spawned
	 */
	...
	public void updateAmmo(int mode, int delta, boolean pickup) {
		...
		case 1:
			...
			if (pickup)
				shotgun_pickup.play(1.0f, 0.5f, false);
			break;
		case 2:
			shieldAmmo = shieldAmmo + delta;
			if (pickup)
				shield_pickup.play(1.0f, 0.5f, false);
			break;
		case 3:
			laserAmmo = laserAmmo + delta;
			if (pickup)
				laser_pickup.play(1.0f, 1.0f, false);
			break;
		case 4:
			multiAmmo = multiAmmo + delta;
			if (pickup)
				multi_pickup.play(1.0f, 1.0f, false);
			break;
		case 5:
			shrapnelAmmo = shrapnelAmmo + delta;
			if (pickup)
				shrapnel_pickup.play(1.0f, 1.0f, false);
			break;
		}
		setAmmoType(mode);
	}
	...
	public void spawnAmmo(float x, float y) {
		...
		Ammo toSpawn;
		if (i > 0 && i < 0.4f)
			toSpawn = new Ammo(shellTexture, 1, shellModel, x, y, 1);
		else if (i >= 0.4f && i < 0.6f)
			toSpawn = new Ammo(shieldTexture, 2, shieldModel, x, y, 1);
		else if (i >= 0.6f && i < 0.75f)
			toSpawn = new Ammo(laserTexture, 3, laserModel, x, y, 1);
		else if (i >= 0.75f && i < 0.9f)
			toSpawn = new Ammo(multiTexture, 4, multiModel, x, y, 1);
		else
			toSpawn = new Ammo(shrapnelTexture, 5, shrapnelModel, x, y, 1);
